...
type ScanResolveResult =
  ...
  | { type: 'duplicate'; duplicate: DuplicateScanDetails | null }
...
const resolveRewardFromCode = async (value?: string): Promise<ScanResolveResult> => {
  ...
  try {
    ...
    if (
      ...
      return { type: 'duplicate', duplicate: normalizeDuplicateScan(payload) };
    }
...
export function ScanScreen({
  ...
  const completeScan = async (data?: string) => {
    ...
    if (resolved.type === 'duplicate') {
      ...
      setDetectedLabel('Already Scanned');
...
const styles = StyleSheet.create({
  ...
  duplicateTitleDark: { color: '#FACC15' },
  duplicateRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    gap: 12,
